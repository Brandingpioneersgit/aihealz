...
const COUNTRIES: { key: CountryKey; label: string; code: string; currency: string; slug: string }[] = [
    { key: 'usa', label: 'United States', code: 'US', currency: 'USD', slug: 'us' },
    { key: 'uk', label: 'United Kingdom', code: 'UK', currency: 'GBP', slug: 'uk' },
    { key: 'india', label: 'India', code: 'IN', currency: 'INR', slug: 'india' },
    { key: 'thailand', label: 'Thailand', code: 'TH', currency: 'THB', slug: 'th' },
    { key: 'mexico', label: 'Mexico', code: 'MX', currency: 'MXN', slug: 'mx' },
    { key: 'turkey', label: 'Turkey', code: 'TR', currency: 'TRY', slug: 'tr' },
    { key: 'uae', label: 'UAE', code: 'AE', currency: 'AED', slug: 'ae' },
];
...
export default async function TreatmentPage({ params }: { params: Promise<{ treatment: string }> }) {
    ...
    const treatmentSchema: Record<string, unknown> = {
        ...
        description: treatmentData?.description || `${displayName} is a ${typeLabel.toLowerCase()} used in ${treatmentData?.specialty || 'General'} medicine.`,
                    ...
                    <header className="col gap-4">
                        ...
                        {simpleName && medicalName && simpleName !== medicalName && (
                            <p className="muted" style={{ fontSize: 14, margin: 0 }}>
                                Medical name: <span style={{ color: 'var(--ink)', fontWeight: 500 }}>{medicalName}</span>
                            </p>
                        )}
                        <p className="lede" style={{ fontSize: 'clamp(16px, 1.6vw, 20px)', maxWidth: 720 }}>
                            Comprehensive guide to {displayName.toLowerCase()} — global cost comparison, procedure details, recovery expectations, and qualified specialists.
                        </p>
                        ...
                        <div className="col gap-6" style={{ minWidth: 0 }}>
                            ...
                            <section className="card col gap-3" style={{ padding: 28 }}>
                                ...
                                <p style={{ fontSize: 15, color: 'var(--ink-2)', lineHeight: 1.65, margin: 0 }}>
                                    {treatmentData?.description || `${displayName} is a ${typeLabel.toLowerCase()} commonly used in ${treatmentData?.specialty || 'general'} medicine. This treatment approach helps address various health conditions and symptoms, providing patients with effective therapeutic options.`}
                                </p>
                                            ...
                                            return (
                                                <li key={i}>
                                                    <Link
                                                        ...
                                                        style={{ padding: '4px 0' }}
                                                    >
                                                        <span style={{ fontSize: 13, color: 'var(--ink)' }}>{t.simpleName || t.name}</span>
                    ...
                    <section className="col gap-4">
                        ...
                        >
                            {[
                                ...
                                {
                                    q: `Is ${displayName} covered by insurance?`,
                                ...
                                {
                                    q: `Are there alternatives to ${displayName}?`,
