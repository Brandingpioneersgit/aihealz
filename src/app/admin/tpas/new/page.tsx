        ...
        <div className="col gap-5" style={{ maxWidth: 720, color: 'var(--ink)' }}>
            ...
            <form onSubmit={submit} className="card col gap-4" style={{ padding: 24 }}>
                ...
                <div className="form-group">
                    ...
                    <select
                        value={form.tpaType}
                        onChange={(e) => setForm({ ...form, tpaType: e.target.value })}
                        className="select"
                    >
                ...
                <div className="form-group">
                    ...
                    <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} rows={3} className="textarea" />
                </div>

                <div className="row gap-3 hairline-t" style={{ paddingTop: 16 }}>
                    <button type="submit" disabled={saving} className="btn btn-cobalt">
                        {saving ? 'Creating…' : 'Create TPA →'}
                    </button>
                    <Link href="/admin/tpas" className="btn btn-paper">Cancel</Link>
                </div>
            </form>
        </div>
    );
